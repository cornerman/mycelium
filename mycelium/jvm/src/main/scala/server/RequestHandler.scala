...
trait RequestHandler[Payload, Event, Failure, State] {
  ...
  def onClientConnect(client: NotifiableClient[Event, State], state: Future[State]): Unit = {}
  ...
  def onClientDisconnect(client: NotifiableClient[Event, State], state: Future[State], reason: DisconnectReason): Unit = {}
  ...
  def onRequest(client: NotifiableClient[Event, State], state: Future[State], path: List[String], payload: Payload): Response
  ...
  def onEvent(client: NotifiableClient[Event, State], state: Future[State], events: List[Event]): Reaction
}
...
trait StatelessRequestHandler[Payload, Event, Failure] extends RequestHandler[Payload, Event, Failure, Unit] {
  ...
  def onRequest(client: NotifiableClient[Event, Unit], path: List[String], payload: Payload): Response
  def onEvent(client: NotifiableClient[Event, Unit], events: List[Event]): Reaction

  final def initialState = Future.successful(())
  final override def onClientConnect(client: NotifiableClient[Event, Unit], state: Future[Unit]): Unit = onClientConnect(client)
  final override def onClientDisconnect(client: NotifiableClient[Event, Unit], state: Future[Unit], reason: DisconnectReason): Unit = onClientDisconnect(client, reason)
  final override def onRequest(client: NotifiableClient[Event, Unit], state: Future[Unit], path: List[String], payload: Payload): Response = onRequest(client, path, payload)
  final override def onEvent(client: NotifiableClient[Event, Unit], state: Future[Unit], events: List[Event]): Reaction = onEvent(client, events)
}
...
trait SimpleStatelessRequestHandler[Payload, Event, Failure] extends StatelessRequestHandler[Payload, Event, Failure] {
  ...
  final override def onClientConnect(client: NotifiableClient[Event, Unit]): Unit = onClientConnect()
  final override def onClientDisconnect(client: NotifiableClient[Event, Unit], reason: DisconnectReason): Unit = onClientDisconnect(reason)
  final override def onRequest(client: NotifiableClient[Event, Unit], path: List[String], payload: Payload): Response = onRequest(path, payload)
  final override def onEvent(client: NotifiableClient[Event, Unit], events: List[Event]): Reaction = ??? // can never be called
}
