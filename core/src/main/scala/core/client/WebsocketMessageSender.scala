...
trait WebsocketMessageSender[PickleType, Sender] {
  ...
  def sendOrBuffer(
    ...
    case None =>
      message match {
        case message: WebsocketMessage.Direct[PickleType] =>
          signalDroppedMessage(message)
        case message: WebsocketMessage.Buffered[PickleType] =>
          queue.append(message)
      }
  }

  def trySendBuffer()(implicit ec: ExecutionContext): Unit =
